...
const Profile = () => {
  ...
  return (
    <div style={{ padding: '140px 24px 80px', maxWidth: '1240px', margin: '0 auto', minHeight: '80vh' }}>
      <div className="section-tag">{t('userConf')}</div>
      <h2 className="section-title">{t('my')} <span className="gradient-text">{t('prof')}</span> {t('settings')}</h2>
      <p className="section-desc">{t('manageAuth')}</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: '40px', marginTop: '40px' }}>
        <div className="price-index-card glass" style={{ padding: '40px' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ background: 'rgba(255,153,51,0.05)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,153,51,0.1)'}}>
              <label style={{ display: 'block', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '1.5px', color: 'var(--saffron)', marginBottom: '12px', fontWeight: '700' }}>{t('secRef')}</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '8px', fontWeight: '500' }}>{t('fullName')}</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '8px', fontWeight: '500' }}>{t('workspaceComp')}</label>
              <input type="text" name="company" value={formData.company} onChange={handleChange} required 
                style={{ width: '100%', padding: '14px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: '12px', color: '#fff', outline: 'none' }} />
            </div>

            <div>
              <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '8px', fontWeight: '500' }}>{t('emailAtlas')}</label>
              <input type="email" name="email" value={formData.email} onChange={handleChange} required 
                style={{ width: '100%', padding: '14px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: '12px', color: '#fff', outline: 'none' }} />
            </div>
            
            <div>
              <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '8px', fontWeight: '500' }}>{t('masterPass')} <span style={{fontSize: '0.7rem', color: 'var(--text-muted)'}}>{t('leaveBlank')}</span></label>
            ...
            <button type="submit" className="btn-primary" style={{ width: '100%', justifyContent: 'center', marginTop: '16px', padding: '16px' }} disabled={loading}>
              {loading ? t('execOver') : t('updateRecs')}
            </button>
          </form>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <div className="glass" style={{ padding: '32px', borderRadius: '24px' }}>
                <h3 style={{ fontSize: '1rem', marginBottom: '20px', color: '#fff' }}>{t('appSettings')}</h3>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        ...
                        <div>
                            <div style={{ fontSize: '0.9rem', fontWeight: '600' }}>{t('soundHaptics')}</div>
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{t('playSubtle')}</div>
                        </div>
                    </div>
                    <button 
                        onClick={toggleSound}
            ...
            <div className="glass" style={{ padding: '32px', borderRadius: '24px' }}>
                 <h3 style={{ fontSize: '1rem', marginBottom: '16px', color: '#fff' }}>{t('needHelp')}</h3>
                 <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '20px' }}>{t('contactOur')}</p>
                 <button className="btn-ghost" style={{ width: '100%', justifyContent: 'center' }}>{t('contactSupport')}</button>
            </div>
        </div>
      </div>
    </div>
